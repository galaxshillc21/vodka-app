...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const distributorDoc = await db.collection("distributors").doc(id).get();

    if (!distributorDoc.exists) {
      return NextResponse.json({ error: "Distributor not found" }, { status: 404 });
    }

    return NextResponse.json({ id: distributorDoc.id, ...distributorDoc.data() });
  } catch (error) {
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updateData = { ...distributorData, updatedAt: new Date().toISOString() };

    if (distributorData.googleMapsUrl) {
    ...
    await db.collection("distributors").doc(id).update(updateData);
