...
const CocktailsPage: React.FC = () => {
	...
	const generateCocktail = async () => {
		...
		try {
			const prompt = `Genera una receta de cóctel única y elegante que utilice Blat Vodka. Ten en cuenta las siguientes preferencias del usuario: "${cocktailInput}". La receta debe incluir el nombre del cóctel, una lista de ingredientes con cantidades (ej: 50 ml de Blat Vodka), y las instrucciones paso a paso. Enfócate en la pureza y suavidad de Blat Vodka. Presenta la respuesta en formato Markdown, con un título para el cóctel, una sección de ingredientes y otra de instrucciones.`;

			const chatHistory = [];
			chatHistory.push({ role: "user", parts: [{ text: prompt }] });

			const payload = { contents: chatHistory };
			const apiKey = "AIzaSyCeQRi50qBnJWxPbcxVOKt1cgPkGoqIDHA"; // La clave API será proporcionada automáticamente por Canvas en tiempo de ejecución.
		...
		html = html.replace(listRegex, (match) => {
			if (match.trim().startsWith('<li class="mb-1">')) {
				...
				return match.includes("1.")
					? `<ol class="list-decimal list-inside pl-4 mb-4">${match}</ol>`
					: `<ul class="list-disc list-inside pl-4 mb-4">${match}</ul`;
			}
			return match;
		});
				...
				{cocktailOutput && (
					<div className="mt-12 p-8 bg-white border border-amber-200 rounded-xl shadow-lg">
						<div
							...
							dangerouslySetInnerHTML={{ __html: parseMarkdown(cocktailOutput) }}
						></div>
					</div>
				)}
			</div>
