...
import { Calendar, MapPin, ExternalLink, Share2, Tag, CalendarPlus, Clock3 } from "lucide-react";
...
export default function EventDetailClient({ eventId }: EventDetailClientProps) {
  ...
  const handleShare = async () => {
    ...
    const shareSummary = [event.name, `${t("eventDate")}: ${dateText}`, `${t("eventTime")}: ${timeText}`, locationText].join(" • ");
    const shareData = {
      title: event.name,
      text: shareSummary,
  ...
  const eventTimeText = formatEventTime(event.date);
        ...
        <div className="basis-2/5 min-h-screen hidden lg:block relative">
          {event.images.length > 0 ? (
            <div className="relative w-full h-full">
              <button type="button" onClick={() => openLightbox(event.images[0], event.name)} className="h-full w-full cursor-zoom-in">
              ...
              <div className="Details space-y-4 mb-8">
                ...
                <div className="flex flex-col md:flex-row align-items-center gap-3">
                  <div className="Left w-full md:w-2/5">
                    ...
                    <p className="text-gray-600 mb-2">{eventDateText}</p>
                    <div className="flex flex-row mt-3 gap-2 items center align-items-center">
                      ...
                      <p className="font-semibold text-gray-900">{t("eventTime")}</p>
                    </div>
                    <p className="text-gray-600 mb-2">{eventTimeText}</p>
                      ...
                      {event.images.map((src, i) => (
                        <CarouselItem key={i} className="basis-auto pl-4">
                          <div className="relative h-[200px] overflow-hidden rounded-md flex items-center justify-start bg-gray-100">
                            <button type="button" onClick={() => openLightbox(src, `${event.name} ${i + 1}`)} className="h-full w-full cursor-zoom-in">
                              <Image src={src} loading="lazy" alt={`${event.name} ${i + 1}`} width={600} height={200} className="h-full w-auto object-contain transition-transform duration-300 hover:scale-105" />
                            </button>
                          </div>
                        </CarouselItem>
                      ))}
