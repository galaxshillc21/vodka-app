...
import { usePathname, useRouter } from "@/src/i18n/navigation"; // Your custom navigation setup
...
import {
	...
	SelectTrigger,
	SelectValue,
} from "@/src/components/ui/select"; // Adjust import path if your shadcn components are elsewhere
...
export default function LocaleSwitcherSelect({ value }: Props) {
	...
	return (
		<div
			className={clsx(
				"relative text-gray-400", // Basic styling for the wrapper
		...
		>
			<Select onValueChange={onValueChange} defaultValue={value} disabled={isPending}>
				<SelectTrigger className="w-[50px] bg-transparent border-none focus:ring-0">
