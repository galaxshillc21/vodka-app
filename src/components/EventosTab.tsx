...
const EventosTab = ({ userCoords, formatDistance }) => {
  const sortedEvents = events
    .map((event) => {
      const eventCoords: [number, number] = [event.location.latitude, event.location.longitude];
      const distance = haversineDistance(userCoords, eventCoords);
            ...
            <li key={event.id} className="frosted-card event relative shadow-md">
              <Tabs defaultValue="info" className="w-full">
                ...
                <TabsContent value="info">
                  <div className="info flex flex-col gap-2">
                    ...
                    <div className="event-location">
                      <span className="distance">{formatDistance(event.distance)}</span> •{" "}
                ...
                <TabsContent value="mapa" className="relative">
                  <Map key={`${event.location.latitude}-${event.location.longitude}`} center={[event.location.longitude, event.location.latitude]} zoom={13} />
